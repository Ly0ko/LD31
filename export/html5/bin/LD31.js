...
$hxClasses["Mob"] = Mob;
Mob.__name__ = ["Mob"];
Mob.__super__ = flixel.FlxSprite;
Mob.prototype = $extend(flixel.FlxSprite.prototype,{
	maxSpeed: null
	,_gravity: null
	,_maxFallSpeed: null
	,_health: null
	,update: function() {
		if(Reg.state.player.x < this.x) this.velocity.set_x(-this.maxSpeed); else this.velocity.set_x(this.maxSpeed);
		flixel.FlxSprite.prototype.update.call(this);
	}
	,kill: function() {
		this.set_alive(false);
...
PlayState.prototype = $extend(flixel.FlxState.prototype,{
	...
	,addLevel: function() {
		...
		this.level.loadMap(openfl.Assets.getText("assets/data/map.csv"),"assets/images/mapTiles.png",16,16);
		this.add(this.level);
	}
	,addPlayer: function() {
...
var Player = function(x,y) {
	...
	this.health = this._health;
	this.bullets = new flixel.group.FlxGroup();
	this.bulletDelay = 5;
};
...
Player.prototype = $extend(flixel.FlxSprite.prototype,{
	...
	,controls: function() {
		...
		if(jumping && (this.touching & 4096) > 0) {
			var finalJumpForce = -(this._jumpforce + Math.abs(this.velocity.x * 0.25));
